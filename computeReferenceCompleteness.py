...
class Align:
    ...
    def __init__(self, contigName, referenceName):
...
def main(argv):
    ...
    circularContigs = collectCircularContigs(args.assembler, args.assemblyFilename, 1000000)
    ...
    for alignFilename in alignFilenames:
        for line in open(alignFilename):
            ...
            alignments.append((contigName, referenceName, alignStart, alignEnd, alignEnd-alignStart, nbMatches))
    ...
    for alignment in alignments:
        ...
        contigName, referenceName, alignStart, alignEnd, alignLength, nbMatches = alignment

        key = contigName + "-" + referenceName
        if not key in contigAlignments:
            contigAlignments[key] = Align(contigName, referenceName)
    ...
    for alignObject in contigAlignments_sorted:
        ...
        if isNewAlign:
            ...
            referenceContigs[referenceName][1].append(contigName)
    ...
    for referenceName in sortedReferenceName:
        ...
        if len(mappedContigs) == 1 and mappedContigs[0] in circularContigs:
            ...
            print(referenceName, "(" + str(len(referenceObject.fragmentHits)), "contigs)" + ": ", len(stats[1]), "contigs", stats[0], aniScore)
            resultFile.write(referenceName + " (" + str(len(referenceObject.fragmentHits)) + " contigs)" + ": " + str(len(stats[1])) + " contigs " + str(stats[0]) + " " + str(aniScore) + "\n")

        for contigName in stats[1]:
...
def computeANI(tmpDir, referenceFilename, mappedContigs, contigFilename):
    ...
    for header, seq in SimpleFastaParser(fileHandle):
        if not header in mappedContigs: continue
